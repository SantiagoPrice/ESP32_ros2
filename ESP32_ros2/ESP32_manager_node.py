...
class ESP32ManagerNode(Node):
    def __init__(self):
        super().__init__('ESP32_manager_node')
        ...
        self.publisher = self.create_publisher(Header, '/ESP32/response', 10)
    ...
    def listener_callback(self, msg):
        ...
        current_bit = msg.data[0] # first bit
        ...
        if chr(current_bit) == 's':
            if not self.isolated:
                    self.cancel_nav2_goal()
            else:
                pass
            self.get_logger().debug("Received stop signal from ESP32, cancelling Nav2 goal if not isolated.")
